...
public class PlayerLevelHighValidator extends BaseRuleExecutor{
	private static final Logger log = Logger.getLogger(PlayerLevelHighValidator.class);
		...
		for(Integer selectedLevel : selectedLevels)
		{
			if(selectedLevel != null)
			{
				if(isRelative)
				{
					if(selectedLevel > 0)
					{
						throw new RuleEngineException(ErrCode.SELECTED_LEVEL_NOT_ALLOWED, "the rule disallow higher tier bet, the relative tier number must less than zero.");
				...
				}else
				{
					...
					{
						throw new RuleEngineException(ErrCode.SELECTED_LEVEL_NOT_ALLOWED, "the rule disallow higher tier bet. selected tiers must less than player's tier.");
					}
				}
